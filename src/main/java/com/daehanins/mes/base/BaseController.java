 ...
 * @author jeonsj
 */
public abstract class BaseController<T, V, ID extends Serializable> {
    ...
    public RestResponse<Page<T>> getPage(@ModelAttribute PageVo param, @RequestParam (name="condition", required=false ) String conditionJson){
        ...
        if (!StringUtils.isBlank(param.getSortColumn())) {
            if(param.isOrderAsc()){
                queryWrapper.orderByAsc(StringUtils.camelToUnderline(param.getSortColumn()));
            } else {
                queryWrapper.orderByDesc(StringUtils.camelToUnderline(param.getSortColumn()));
            }
        }
    ...
    @ResponseBody
    public RestResponse<T> save(@ModelAttribute T entity){
        T data;
        if (getTableService().saveOrUpdate(entity)) {
            data = entity;
        } else {
            data = null;
        }
        return new RestUtil<T>().setData(data);
    }

    @RequestMapping(value = "/saveJson", method = {RequestMethod.POST, RequestMethod.PUT})
    @ResponseBody
    public RestResponse<T> saveJson(@RequestBody T entity){
        T data;
        if (getTableService().saveOrUpdate(entity)) {
            data = entity;
        } else {
    ...
    @RequestMapping(value = "/delByIds/{ids}",method = {RequestMethod.DELETE, RequestMethod.POST})
    @ResponseBody
    public RestResponse<Object> delAllByIds(@PathVariable ID[] ids){
        String msg = "";
        ...
        return new RestUtil<Object>().setMessage(msg);
    }
}
